<?php

return [
    'title' => 'Tankoj!',
    'subtitle' => 'A relative-complex tanks game',
    'content' => "<p class='mb-2'>Everything made from scratch from the graphics engine to the UI and designs except the backgrounds.</p>
<p class='mb-2'><a href='https://github.com/EGA-SUPREMO/RPG-Game-template'>RPG Game Template</a> made possible this game.</p>
<p class='mb-2'>Similar to <a href='https://store.steampowered.com/app/70640/Worms/'>Worms inc.</a>.</p>",
    'stack' => 'Java',
    'link' => 'https://github.com/EGA-SUPREMO/Tankoj',
];
